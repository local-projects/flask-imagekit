...
class ImageCacheFile(BaseIKFile):
    ...
    def __init__(self, generator, name=None, storage=None, cachefile_backend=None, cachefile_strategy=None):
        ...
        if not name:
            try:
                ...
                fn = get_by_qname(conf.IMAGEKIT_CACHEFILE_NAMER, 'namer')
                name = fn(generator)
        self.name = name

        storage = storage or getattr(generator, 'cachefile_storage',
            None) or get_singleton(conf.IMAGEKIT_DEFAULT_FILE_STORAGE,
            'file storage backend')
        self.cachefile_backend = (
            cachefile_backend
            or getattr(generator, 'cachefile_backend', None)
            or get_singleton(conf.IMAGEKIT_DEFAULT_CACHEFILE_BACKEND,
                             'cache file backend'))
        self.cachefile_strategy = (
            cachefile_strategy
            or getattr(generator, 'cachefile_strategy', None)
            or get_singleton(conf.IMAGEKIT_DEFAULT_CACHEFILE_STRATEGY,
                             'cache file strategy')
        )
    ...
    def _generate(self):
        ...
        # TODO - Figure out Django File alternative
